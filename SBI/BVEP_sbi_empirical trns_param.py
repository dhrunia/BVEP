...
import torch
import numpy as np
from BVEP_Simulator import VEP2Dmodel, VEP2Dmodel_trans_param
...
def VEP2Dmodel_seeg_simulator_wrapper(params):
    ...
    src_sig = VEP2Dmodel_trans_param(params, constants, strct_con)
...
for _ in range(n_rounds):
    theta, x = simulate_for_sbi(simulator, proposal, 
                                num_simulations=10000, show_progress_bar=True)
...
posterior_samples = posterior.sample(
    ...
    mcmc_parameters={'warmup_steps':1000}).numpy()
# %%
...
pred_summ_stats_syn = VEP2Dmodel_seeg_simulator_wrapper(pstr_samples_mean)
